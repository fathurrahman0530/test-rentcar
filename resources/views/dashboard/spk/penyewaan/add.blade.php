...
@section('btn.nav')
    <a href="{{ route('db.spk.penyewaan') }}" class="btn btn-secondary" style="float: left !important;">
...
@section('content')
    <div class="page-content">
        <div class="container-fluid">
            ...
            <form action="{{ route('db.spk.penyewaan.store') }}" method="POST">
                @csrf
                <input type="hidden" name="act" value="Store">
                <input type="hidden" name="no_spk" value="{{ isset($data['lastNoSpk']->no_spk) ? sprintf("%04d", intval($data['lastNoSpk']->no_spk) + 1) : '0001' }}">
                <div class="row">
                    <div class="col-lg-6 mb-3 mt-4">
                        <div class="card">
                            <h3 class="card-header">Data Mobil</h3>
                            <input type="hidden" id="uuid_penyewaan" name="uuid" value="{{ \Ramsey\Uuid\Uuid::uuid4()->toString() }}">
                                    ...
                                    <div class="col-lg-6 mb-3 mb-3">
                                        ...
                                        <select class="cars form-control" data-trigger name="no_polisi" id="choices-single-default">
                                            ...
                                            @foreach ($data['mobil'] as $m )
                                                <option value="{{ $m->uuid }}">{{ $m->plat }} | {{ $m->brand_name }} | {{ $m->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-lg-6 mb-3 mb-3">
                                    ...
                                    <div class="col-lg-6 mb-3">
                                        ...
                                        <select class="nik form-control " data-trigger name="nik" id="choices-single-default">
                                            ...
                                            @foreach ($data['pelanggan'] as $p )
                                                <option value="{{ $p->IDUser }}">{{ $p->ktp }} | {{ $p->fullname }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-lg-6 mb-3">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-lg-6 mb-3">
                                                ...
                                                <select class="driver form-control " data-trigger name="driver" id="choices-single-default">
                                                    <option value="">This is a placeholder</option>
                                                    @foreach ($data['pelanggan'] as $p )
                                                        <option value="{{ $p->IDUser }}">{{ $p->ktp }} | {{ $p->fullname }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </div>
    ...
    <div id="addPelanggan" class="modal fade" tabindex="-1" aria-labelledby="myModalLabel" aria-hidden="true"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('db.master.pelanggan.storePenyewa') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="act" value="Store">
                    ...
                    success: function (data) {
                        ...
                        if (data > 0) {
                            if (window.confirm(`Pelanggan memiliki tunggakan ${data}, ingin lanjutkan?`)) {
                                ...
                                window.location.href = "{{ route('db.spk.penyewaan') }}";
                            }
                        }
                    }
                });
